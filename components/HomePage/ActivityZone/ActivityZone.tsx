...
const ActivityZone = () => {
    ...
    return (
        <section className="py-16 relative flex items-center justify-center">
            <div className="md:w-[90%]">

                <div className="container mx-auto px-4">
                    {/* Header */}
                    <div className="text-center mb-12">
                        <h2 className="text-4xl md:text-5xl font-extrabold text-purple mb-6">12 different activity zones</h2>
                    </div>
                    ...
                    <div className="relative overflow-hidden">
                        ...
                        >
                            {extendedActivities.map((activity, index) => (
                                <div key={`${activity.title}-${index}`} className="w-1/3 flex-shrink-0 p-2">
                        ...
                        {/* Navigation Arrows */}
                        {/* <button
                            className="absolute left-4 top-1/2 transform -translate-y-1/2 bg-purple rounded-full p-3 shadow-lg hover:shadow-xl transition-all duration-300 z-10"
                        ...
                        <button
                            className="absolute right-4 top-1/2 transform -translate-y-1/2 bg-purple rounded-full p-3 shadow-lg hover:shadow-xl transition-all duration-300 z-10"
                            onClick={handleNext}
                    ...
                    {/* Navigation Dots */}
                    <div className="flex justify-center items-center mt-8 space-x-2 gap-8">
                        <button
                            className="bg-purple rounded-full p-3 shadow-lg hover:shadow-xl transition-all duration-300 z-10 cursor-pointer"
                            onClick={handlePrevious}
                            aria-label="Previous slide"
                        >
                            <svg className="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div className="flex justify-center space-x-2">
                            {activities.map((_, index) => (
                                <button
                                    key={index}
                                    className={`w-3 h-3 rounded-full transition-all duration-300 ${Math.floor((currentIndex - 5) % activities.length) === index
                                        ? "bg-purple scale-125"
